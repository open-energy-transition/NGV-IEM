...
selected_data = "Average congestion income [€/h]"


data = pd.DataFrame({
        'Status_Quo': df_sq[selected_data], # Adjust column selection if needed
        'IEM': df_iem[selected_data]
    })
...
limit_min = min(data.min())
limit_max = max(data.max())
...
# TODO put labels on each scatter point (which interconnector)
for idx, row in data.iterrows():
    plt.text(row['Status_Quo'], row['IEM'], idx,
             fontsize=9, ha='right', va='bottom')
